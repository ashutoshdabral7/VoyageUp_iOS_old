...
-(void)addViewControllers:(UIViewController *) controller needToRefresh:(BOOL) refresh{
    ...
    if(controller.title){
        ...
        v1=controller.navigationItem.titleView;
    }
    else{
        UILabel *item = [UILabel new];
...
-(void)tapOnHeader1:(UITapGestureRecognizer *)recognizer{
    if(self.isUserInteraction){
        // Get the wanted view
        UIView *view = [self.viewControllers objectForKey:@(recognizer.view.tag)];
        [self.scrollView scrollRectToVisible:view.frame
...
-(void)updateNavItems:(CGFloat) xOffset{
    __block int i = 0;
    //NSLog(@"self nav= %@",self.navItemsViews);
    UIImage *logo = [UIImage imageNamed:@"hom"];
    logo = [logo imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    UIImageView* imageView = [[UIImageView alloc] initWithImage:logo];
    UIView *v=[self.navItemsViews objectAtIndex:2];
    imageView.frame=CGRectMake(v.frame.origin.x, v.frame.origin.y, v.frame.size.width, v.frame.size.height);
   // [self.navItemsViews replaceObjectAtIndex:1 withObject:imageView];
